...
function calculateSum(n) {
  ...
  let i = 1;           // Запустити цикл для змінної i від 1 до n
  while (i <= n) {
    sum = sum + i;     // Додати поточне значення i до змінної sum
    i++;
  }
...
function isPrimeNumber(num) {
   if (num <= 1) {
    ...
    for (let i = 2; i < num - 1; i++) {   // Запускаємо цикл для змінної i від 2 до num-1
      if (num % i === 0) {                // Перевіряємо, чи num ділиться націло на поточне значення i
...
function getUserStatus(age) {
   ...
   if (age < 18) {                      // Якщо age менше 18, то користувач є "Неповнолітнім"           
    return `Неповнолітній`;               
  } else if (age >= 18 && age < 65) {   // Якщо age більше або дорівнює 18 і менше 65, то користувач є "Дорослим"
    return `Дорослий`;                    
  } else if (age >= 65) {               // В іншому випадку (якщо age більше або дорівнює 65), то користувач є "Пенсіонером"
    return `Пенсіонер`;
  }
}
...
function getDayOfWeek(dayNumber) {
 ...
 let day = 0;
  switch (dayNumber) {        // Використовуємо оператор switch для знаходження назви дня тижня залежно від dayNumber
...
typeof variable;                         // Отримуємо тип змінної variable за допомогою оператора typeof
  return typeof variable === "number"    // Встановлюємо відповідне повідомлення залежно від отриманого типу  
...
function factorial(n) {
   ...
   if (n === 0 || n === 1) {        // Якщо n дорівнює 0 або 1, повертаємо 1, оскільки факторіал 0 і 1 завжди дорівнює 1.
    return 1;
  } else {
...
function makeAdder(x) {
  ...
  return function newfuncion(y) {      // Вертаємо нову функцію, яка приймає `y`
...
function divide(x) {
  ...
  return function divide(y) {    // Повертаємо нову функцію, яка приймає `y`
...
function memoizedArea() {
  ...
  return (side) => {            // Повертаємо функцію яка приймає аргумент side - сторону квадрата
    if (side === prevSide) {    // Перевіряємо, чи є введена сторона такою ж, як і в попередньому виклику
      console.log(`Fetching from cache ${prevSquare}`);   // Виводимо повідомлення Fetching from cache
      return prevSquare;        // Повертаємо попереднє значення
    } 
    ...
    const square = side * side;    // Розраховуємо площу
    prevSide = side;                // prevSide присвоюємо сторону квадрата
    prevSquare = square;            // prevResult просвоюємо площу квадрата

    console.log(`Calculating result ${square}`);    // Виводимо повідомлення Calculating result
    return square;                   // Повертаємо площу
  };
